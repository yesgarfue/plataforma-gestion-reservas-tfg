...
timestamp: 2026-05-14T19:51:12+02:00
...
| RF-08 | El sistema permite búsqueda de barcos por nombre o título desde la página de inicio. | Alta |
| RF-09 | El sistema permite filtrar barcos por puertos, fabricantes, precio, categoría, capacidad y rango de fechas de forma combinable e independiente. | Alta |
| RF-10 | Los filtros de puerto y fabricante se presentan como desplegables con valores válidos. | Alta |
| RF-11 | El filtro de fechas muestra todos los barcos del catálogo, marcando con etiqueta 'No disponible' aquellos no disponibles en ese rango sin ocultarlos. | Alta |
| RF-12 | Los barcos agotados o no disponibles aparecen claramente marcados en el catálogo. | Alta |
| RF-13 | El cliente puede acceder a la ficha del barco desde el catálogo, visualizando los datos del barco e imagen. | Alta |
...
| RF-22 | Durante el proceso de reserva se solicitan los datos del cliente y datos de pago, heredando datos si hay sesión iniciada. | Alta |
| RF-23 | El sistema ofrece PayPal Sandbox como método de pago. | Alta |
| RF-24 | El sistema ofrece contra-reembolso como método de pago. | Alta |
| RF-25 | Al finalizar la reserva, el cliente recibe un correo electrónico con datos del barco, rango de fechas, importe total y código de seguimiento. | Alta |
| RF-26 | El sistema aplica una tasa de combustible de 50 € por día, excepto para barcos de categoría velero donde la tasa es 0 €. | Alta |
| RF-27 | Cada reserva tiene uno de los estados: PENDIENTE DE PAGO, PAGADO, EN USO o DEVUELTO. | Alta |
| RF-28 | Una reserva puede cancelarse únicamente si está en estado PENDIENTE DE PAGO, siendo eliminada del sistema sin dejar registro posterior. | Alta |
| RF-29 | Si falta un día para el inicio de la reserva y sigue en estado PENDIENTE DE PAGO, el sistema envía un correo de recordatorio al usuario. | Alta |
| RF-30 | El administrador puede cambiar el estado de una reserva desde el panel de reservas mediante botones por transición aplicable. | Alta |
| RF-31 | El cliente puede consultar el estado de su reserva usando el código de seguimiento recibido por email, incluso sin estar registrado. | Alta |
| RF-32 | El usuario registrado puede consultar el estado de sus reservas desde su cuenta. | Alta |
| RF-33 | El administrador puede consultar el estado de cualquier reserva. | Alta |
| RF-34 | El sistema proporciona un panel de administración propio para gestión de barcos, clientes y reservas. | Alta |
| RF-35 | El administrador puede realizar alta, edición y baja de barcos desde el panel de administración. | Alta |
| RF-36 | El administrador puede consultar y eliminar clientes desde el panel de administración, respetando la restricción de no eliminar usuarios con reservas pendientes. | Alta |
| RF-37 | El administrador puede consultar y cambiar el estado de reservas desde el panel de administración. | Alta |
